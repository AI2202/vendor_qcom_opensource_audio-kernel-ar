...
static int cs35l45_supported_devid(struct cs35l45_private *cs35l45)
{
	...
	int ret = 0;
	/* ASUS_BSP Set Level 1 Current threshold from default 0x2C 3A to 0x36 3.625A */
	...
	/* ASUS_BSP Set Level 1 Current threshold from default 0x2C 3A to 0x36 3.625A +++ */
	ret = regmap_read(cs35l45->regmap, CS35L45_BST_BPE_IL_LIM_THLD, &Current_threshold);
	if (ret < 0)
		dev_err(cs35l45->dev, "Get Level 1 Current threshold failed\n");
	else
		dev_err(cs35l45->dev, "Cirrus Logic CS35L45: Level 1 Current threshold 0x%02X.\n", Current_threshold);
	/* ASUS_BSP Set Level 1 Current threshold from default 0x2C 3A to 0x36 3.625A --- */
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	regmap_write(cs35l45->regmap, CS35L45_BST_BPE_IL_LIM_THLD, 0x36);

	ret = regmap_read(cs35l45->regmap, CS35L45_BST_BPE_IL_LIM_THLD, &Current_threshold);
	if (ret < 0)
		dev_err(cs35l45->dev, "Get Level 1 Current threshold failed again\n");
	else
		dev_err(cs35l45->dev, "Cirrus Logic CS35L45: Reset Level 1 Current threshold to 0x36(0x%02X).\n", Current_threshold);
	/* ASUS_BSP Set Level 1 Current threshold from default 0x2C 3A to 0x36 3.625A --- */
